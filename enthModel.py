...
rhoS  = Expression(code, Tavg=Tavg, A=A, Va=Va)
...
theta     = 1.0
f_T       = (rho*ci*(T-T_0)*psi/dt + \
            theta*k*inner(grad(T),grad(psi)) + \
            theta*rho*ci*w*grad(T)*psi + \
            theta*dkdz*grad(T)*psi + \
            (1-theta)*km1*inner(grad(T_0),grad(psi)) + \
            (1-theta)*rho_0*ci*wm1*grad(T_0)*psi+ \
...
drhokdt   = (rhoi - rho)*(acc*rhoi/rhow)*beta*K
drhok_0dt = (rhoi - rho_0)*(acc*rhoi/rhow)*beta*K

# theta scheme (1=Backwards-Euler, 0.5=Crank-Nicolson, 0=Forward-Euler) :
theta     = 1.0
f_rho     = ((rho-rho_0)/dt - \
            theta*(drhokdt - w*grad(rho)) - \
            (1-theta)*(drhok_0dt - wm1*grad(rho_0)))*phi*dx
...
phT,    = Tax.plot(tplot[index] - 273.15, z[index], 'r-')    # temp plot
