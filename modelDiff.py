...
rho   = ones(n) * rhos         # density vector - kg/m^3
...
def calc_variables(y, rhom2, rhom1, rho, k, w):
  ...
  # velocity :
  w      = - acc / rho
  # density ( drho / dt = (rho - rhom1) / dt ) :
  rho    = rhom1 + dt*( K*acc*(rhoi - rho)/rhoi - w*(B * rho) )
  # density ( drho / dt = (rhom2 - 4*rhom1 + 3*rho) / 2dt ) :
  #rho    = 4*rhom1 - rhom2 + \
